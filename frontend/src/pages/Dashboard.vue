  ...
  <div class="page">
    ...
    <div v-else-if="report">
      <div class="stat-grid">
        ...
        <div class="stat-card"><div class="stat-label">Commissions</div><div class="stat-value red">{{ fmt(report.total_commissions) }}</div></div>
        <div class="stat-card"><div class="stat-label">Expenses</div><div class="stat-value red">{{ fmt(report.total_expenses) }}</div></div>
      ...
      <div v-if="report.payment_breakdown" class="section">
        ...
        <div class="payment-grid">
          <div class="payment-card cash">
            ...
            <div class="payment-value">{{ fmt(report.payment_breakdown.cash) }}</div>
          </div>
          <div class="payment-card card">
            <div class="payment-label">Card</div>
            <div class="payment-value">{{ fmt(report.payment_breakdown.card) }}</div>
          </div>
          <div class="payment-card online">
            <div class="payment-label">Online</div>
            <div class="payment-value">{{ fmt(report.payment_breakdown.online) }}</div>
          </div>
        </div>
      </div>
      <div v-if="report.staff_summary?.length" class="section">
        ...
        <table class="data-table">
          <thead><tr><th>Staff</th><th>Bills</th><th>Sales</th><th>Commission</th></tr></thead>
          <tbody>
            <tr v-for="s in report.staff_summary" :key="s.staff_id">
              <td>{{ s.staff_name }}</td><td>{{ s.bill_count }}</td><td>{{ fmt(s.total) }}</td><td class="red">{{ fmt(s.commission) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
.payment-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 1rem; }
.payment-card { border-radius: 8px; padding: 1rem 1.25rem; }
.payment-card.cash { background: #dcfce7; }
.payment-card.card { background: #dbeafe; }
.payment-card.online { background: #ede9fe; }
.payment-label { font-size: 0.8rem; font-weight: 600; margin-bottom: 0.25rem; color: #374151; }
.payment-value { font-size: 1.2rem; font-weight: 700; color: #1a1a2e; }
